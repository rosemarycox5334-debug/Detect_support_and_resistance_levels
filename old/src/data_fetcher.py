...
from config import (
    DATA_DIR, MAX_RETRIES, RETRY_BACKOFF, REQUEST_TIMEOUT,
...
class DataFetcher:
    ...
    def get_stock_codes(self, limit: int = 0, exclude_st: bool = True) -> List[str]:
        """
        获取股票代码列表
        ...
        返回:
            代码列表，如 ['sh.600519', 'sz.000001', ...]
        """
        sl = self.get_stock_list(exclude_st=exclude_st, only_active=True)
        codes = sl["code"].tolist()
        if limit > 0:
            # 按代码排序后取前 limit 个（保证可复现）
            codes.sort()
            codes = codes[:limit]
        return codes
    ...
    def batch_get_kline(
        ...
        for i, code in enumerate(codes):
            try:
                df = self.get_kline(code, freq=freq, start=start, end=end)
                if len(df) > 0:
                    result[code] = df
            except Exception as e:
                print(f"  [跳过] {code}: {e}")
...
if __name__ == "__main__":
    ...
    try:
        # 测试单股K线
        df = fetcher.get_kline("600519", start="2024-01-01", end="2024-12-31")
        print(f"贵州茅台 日K线: {len(df)} 条")
        print(df.head())
        print()
